...
extension Range where Bound: BinaryFloatingPoint & Comparable {
  ...
  ///
  /// - Returns: The magnitude of the range as a `Bound` value, representing the distance between `upperBound` and `lowerBound`.
...
extension ClosedRange where Bound: BinaryFloatingPoint & Comparable {
  ...
  /// It is useful for determining the size or extent of the closed range when working with floating-point values.
  ///
  /// - Returns: The magnitude of the closed range as a `Bound` value, representing the distance between `upperBound` and `lowerBound`.
  public var magnitude: Bound { upperBound - lowerBound }
}
