...
/// A collection wrapper that provides both the index and element of each entry in a `RandomAccessCollection`.
///
/// `IndexedCollection` is a generic struct that takes any `RandomAccessCollection` and provides access to both
/// the index and the element of each entry in the collection. This is particularly useful when you need
...
struct IndexedCollection<Base: RandomAccessCollection>: RandomAccessCollection {
  ...
  /// - Returns: An index offset by `distance` from the index `i`. If `distance` is positive, this is the same value that would be returned by the default implementation of `index(_:offsetBy:)`. If `distance` is negative, this is the same value that would be returned by the default implementation of `index(_:offsetBy:)`.
  ...
  ///
  /// This subscript provides access to the element and its index in the collection. The returned tuple contains
